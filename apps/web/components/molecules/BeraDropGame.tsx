...
import { useApplications, useBeraDrop } from '../../contexts';
...
export function BeraDropGame(_props: Props) {
  const { isLoaded, unityProvider, unload } = useBeraDrop();
  const { applications, setApplications } = useApplications();
  const { isTouch } = useTouchDevice();
  const application =
    applications.find((app) => app.id === Applications.BERA_DROP) ||
    ApplicationData[Applications.BERA_DROP];

  async function handleCloseWindow(event?: MouseEvent<HTMLButtonElement>) {
    event?.stopPropagation();
    setApplications(
      applications.filter((app) => app.id !== Applications.BERA_DROP),
    );
    await unload();
  }
