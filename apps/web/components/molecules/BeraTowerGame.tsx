...
import { useApplications, useBeraTower } from '../../contexts';
...
export function BeraTowerGame(_props: Props) {
  const { isLoaded, unityProvider, unload } = useBeraTower();
  const { applications, setApplications } = useApplications();
  const { isTouch } = useTouchDevice();
  const application =
    applications.find((app) => app.id === Applications.BERA_TOWER) ||
    ApplicationData[Applications.BERA_TOWER];

  async function handleCloseWindow(event?: MouseEvent<HTMLButtonElement>) {
    event?.stopPropagation();
    setApplications(
      applications.filter((app) => app.id !== Applications.BERA_TOWER),
    );
    await unload();
  }
